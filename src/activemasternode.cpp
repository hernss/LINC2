...
void CActiveMasternode::ManageStateInitial()
{
    ...
    if (chainActive.Height() < Params().GetConsensus().nBlockNewCollateralStart) {
        if (pwalletMain->GetBalance() < Params().GetConsensus().OldCollateralAmount * COIN) {
            LogPrintf("CActiveMasternode::ManageStateInitial -- %s: Wallet balance is < 2500 LINC\n", GetStateString());
            return;
        }
    } else {
        if (pwalletMain->GetBalance() < Params().GetConsensus().NewCollateralAmount * COIN) {
            LogPrintf("CActiveMasternode::ManageStateInitial -- %s: Wallet balance is < %i LINC\n", GetStateString(), Params().GetConsensus().NewCollateralAmount);
            return;
        }
    }
