...
CAmount CWalletTx::GetAnonymizedCredit(bool fUseCache) const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        if (pwallet->IsSpent(hashTx, i) || !pwallet->IsDenominated(txin))
            continue;
...
CAmount CWallet::GetAnonymizableBalance(bool fSkipDenominated) const
{
    ...
    if (!SelectCoinsGrouppedByAddresses(vecTally, fSkipDenominated))
        return 0;
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (IsSpent(hash, i) || IsMine(pcoin->vout[i]) != ISMINE_SPENDABLE || !IsDenominated(txin))
                    continue;
...
CAmount CWallet::GetNeedsToBeAnonymizedBalance(CAmount nMinBalance) const
{
    ...
    if (nNeedsToAnonymizeBalance > PRIVATESEND_POOL_MAX)
        nNeedsToAnonymizeBalance = PRIVATESEND_POOL_MAX;
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if (nCoinType == ONLY_DENOMINATED) {
                    ...
                    if (nHeight < Params().GetConsensus().nBlockNewCollateralStart)
                        found = !(fMasterNode && pcoin->vout[i].nValue == Params().GetConsensus().OldCollateralAmount * COIN);
                    else
                        found = !(fMasterNode && pcoin->vout[i].nValue == Params().GetConsensus().NewCollateralAmount * COIN);
                    ...
                    if (nHeight < Params().GetConsensus().nBlockNewCollateralStart) {
                        if (found && fMasterNode)
                            found = pcoin->vout[i].nValue != Params().GetConsensus().OldCollateralAmount * COIN; // do not use Hot MN funds
                    } else {
                        if (found && fMasterNode)
                            found = pcoin->vout[i].nValue != Params().GetConsensus().NewCollateralAmount * COIN; // do not use Hot MN funds
                    }

                } else if (nCoinType == ONLY_2500) {
                    if (nHeight < Params().GetConsensus().nBlockNewCollateralStart)
                        found = pcoin->vout[i].nValue == Params().GetConsensus().OldCollateralAmount * COIN;
                    else
                        found = pcoin->vout[i].nValue == Params().GetConsensus().NewCollateralAmount * COIN;
...
bool CWallet::SelectCoinsGrouppedByAddresses(std::vector<CompactTallyItem>& vecTallyRet, bool fSkipDenominated, bool fAnonymizable) const
{
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        ...
        if (wtx.IsCoinBase() && wtx.GetBlocksToMaturity() > 0)
            continue;
        if (!fAnonymizable && !wtx.IsTrusted())
            continue;
        ...
        for (unsigned int i = 0; i < wtx.vout.size(); i++) {
            ...
            if (IsSpent(wtx.GetHash(), i) || IsLockedCoin(wtx.GetHash(), i))
                continue;

            if (fSkipDenominated && IsDenominatedAmount(wtx.vout[i].nValue))
                continue;

            if (fAnonymizable) {
                // ignore collaterals
                if (IsCollateralAmount(wtx.vout[i].nValue))
                    continue;
                if (chainActive.Height() < Params().GetConsensus().nBlockNewCollateralStart) {
                    if (fMasterNode && wtx.vout[i].nValue == Params().GetConsensus().OldCollateralAmount * COIN)
                        continue;
                } else {
                    if (fMasterNode && wtx.vout[i].nValue == Params().GetConsensus().NewCollateralAmount * COIN)
                        continue;
				}
                // ignore outputs that are 10 times smaller then the smallest denomination
                // otherwise they will just lead to higher fee / lower priority
                if (wtx.vout[i].nValue <= vecPrivateSendDenominations.back() / 10)
                    continue;
                // ignore anonymized
                if (GetInputPrivateSendRounds(CTxIn(wtx.GetHash(), i)) >= nPrivateSendRounds)
                    continue;
            }
    ...
    BOOST_FOREACH (const PAIRTYPE(CBitcoinAddress, CompactTallyItem) & item, mapTally) {
        if (fAnonymizable && item.second.nAmount < vecPrivateSendDenominations.back())
            continue;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax) const
{
    ...
    BOOST_FOREACH (const COutput& out, vCoins) {
        ...
        if (out.tx->vout[out.i].nValue < nValueMin / 10)
            continue;
            ...
            if (pcoin->IsTrusted()) {
                ...
                for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                    ...
                    if (!IsDenominatedAmount(pcoin->vout[i].nValue))
                        continue;
                    if (IsSpent(out.tx->GetHash(), i) || IsMine(pcoin->vout[i]) != ISMINE_SPENDABLE || !IsDenominated(txin))
                        continue;
...
bool CWallet::CreateTransaction(const vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, int& nChangePosRet, std::string& strFailReason, const CCoinControl* coinControl, bool sign, AvailableCoinsType nCoinType, bool fUseInstantSend)
{
